...
class WebhookController extends CommonAjaxController
{
    ...
    public function processAction(Request $request)
    {
        ...
        foreach($hooks as $hook){
            if(!empty($data[$hook])){
                foreach($data[$hook] as $hookData){
                    if(!empty($hookData['contact'])) {
                        $recombeeHelper->getClient()->send(new SetUserValues($hookData['contact']['id'], $recombeeHelper->parseContactData($hookData['contact']), [
                            'cascadeCreate' => true
                        ]));
                    }
                }
